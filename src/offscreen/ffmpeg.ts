...
/**
 * Re-encodes a finished compilation to H.264/AAC MP4, for scouts who want
 * an MP4 to hand off elsewhere rather than reviewing/sharing WebM directly.
 * Not a stream copy: WebM's codecs (VP9/Opus) aren't broadly playable
 * inside an MP4 container, so this is a genuine decode + re-encode pass —
 * the same trade-off as correctPlaybackSpeed above, and reusing its
 * wasm-appropriate encoder settings (see that function's comment for why
 * the defaults are far too slow here).
 *
 * This is NOT subject to the mid-recording MP4 problems that pushed the
 * *recording* format to WebM (see CANDIDATE_MIME_TYPES in types.ts): those
 * were specifically about a file peeked at before MediaRecorder finalizes
 * it (no moov atom yet) and about concatenating multiple independent
 * recorder segments (DTS discontinuity at the seams). This runs once,
 * on one single already-finished, already-concatenated file — the same
 * shape of operation as correctPlaybackSpeed, which has been reliable
 * throughout.
 */
export function convertToMp4(blob: Blob, sourceExtension: string): Promise<Blob> {
  if (sourceExtension === 'mp4') return Promise.resolve(blob)
  return queued(() => convertToMp4Impl(blob, sourceExtension))
}

async function convertToMp4Impl(blob: Blob, sourceExtension: string): Promise<Blob> {
  const ffmpeg = await getFFmpeg()
  const input = `mp4in.${sourceExtension}`
  const output = 'mp4out.mp4'
  try {
    await ffmpeg.writeFile(input, new Uint8Array(await blob.arrayBuffer()))
    await execChecked(ffmpeg, [
      '-i',
    ...
    return await readOutputBlob(ffmpeg, output, 'video/mp4')
  } finally {
    await ffmpeg.deleteFile(input).catch(() => {})
    await ffmpeg.deleteFile(output).catch(() => {})
  }
